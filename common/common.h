...
struct common_params_sampling {
    ...
    // escalating n-gram penalty against fixed reference sequences (common/ngram-penalty.h)
    std::vector<std::vector<llama_token>> ngram_penalty_refs;
    int32_t ngram_penalty_start_n = 3;    // first match length that is penalized
    float   ngram_penalty_scale   = 0.0f; // 0 = disabled; penalty = (n - start_n + 1) * scale
    float   ngram_penalty_max     = 10.0f;
