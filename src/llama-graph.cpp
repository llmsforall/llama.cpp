...
static bool rs_mapping_is_contiguous(const llama_memory_recurrent_context * mctx, int64_t n_seqs);
...
bool llm_graph_input_rs::can_reuse(const llm_graph_params & params) {
    ...
    // the graph hard-codes view-vs-gather for the state read
    res &= rs_contiguous == rs_mapping_is_contiguous(mctx, params.ubatch.n_seqs);
...
ggml_tensor * llm_graph_context::build_rs(
    ...
    // When the mapping is the identity (steady-state decode) the gather would
    // copy each row onto itself: read the rows in place instead. At 32 seqs
    // this avoids ~1.9 GB of reads plus 1.9 GB of writes per token step.
    ggml_tensor * output_states;
    if (contiguous) {
        output_states = ggml_view_2d(ctx0, states, state_size, n_seqs,
                                     states->nb[1], rs_head*states->nb[1]);
    } else {
...
// The state "gather" is the identity whenever cell i's source is cell i, which
// is the steady-state decode case (see llama_memory_recurrent::commit: src is
// reset to the cell's own index after every step). Detecting it lets build_rs
// use a view of the state buffer instead of copying it.
static bool rs_mapping_is_contiguous(const llama_memory_recurrent_context * mctx, int64_t n_seqs) {
    if (getenv("GGML_RS_VIEW") && atoi(getenv("GGML_RS_VIEW")) == 0) {
        return false;
    }
    const int32_t head = (int32_t) mctx->get_head();
    for (int64_t i = 0; i < n_seqs; ++i) {
        if (mctx->s_copy((int) i) != head + (int32_t) i) {
            return false;
        }
    }
    return true;
...
static std::unique_ptr<llm_graph_input_rs> build_rs_inp_impl(
    ...
    inp->rs_contiguous = rs_mapping_is_contiguous(mctx_cur, n_seqs);
...
ggml_tensor * llm_graph_context::build_rs(
    ...
    return build_rs(s, inp->s_copy_main, inp->s_copy_extra, state_size, n_seqs,
                    ...
                    get_state_rows, inp->rs_contiguous);
}
