...
public:
    ...
    // true when s_copy_main is the identity mapping [head, head+1, ...], i.e.
    // the "gather" would copy every state row onto itself. Steady-state decode
    // always hits this: llama_memory_recurrent sets cells[i].src = i after each
    // step, so the next step's src0 == i. build_rs then takes a view of the
    // state buffer instead of materializing a copy of it. Part of graph reuse
    // validity: a graph built with the view must not be reused when the
    // mapping stops being contiguous (sequence moved slots, or a fresh
    // sequence needs the zeroed cell).
    bool rs_contiguous = false;
};
