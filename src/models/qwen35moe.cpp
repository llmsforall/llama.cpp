...
ggml_tensor * llm_build_qwen35moe ::build_layer_attn_linear(
    ...
    // Output projection.
    // Collapse (n_seq_tokens, n_seqs) into a single token axis first: with a 3D
    // src1 the projection runs as n_seqs channel-matmuls of n_seq_tokens
    // columns each, and each channel re-reads the whole weight matrix. At
    // decode (n_seq_tokens == 1) that is one 4.85 MB read per sequence instead
    // of one for the whole batch. The tensor is contiguous, so this is free,
    // and the result already has the shape the caller wants.
    ggml_tensor * final_output_2d =
        ggml_reshape_2d(ctx0, final_output, head_v_dim * num_v_heads, n_seq_tokens * n_seqs);

    cur = build_lora_mm(model.layers[il].ssm_out, final_output_2d, model.layers[il].ssm_out_s);
