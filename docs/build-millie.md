...
For a prebuilt runtime and complete model-start commands, use the
[standalone quickstart](../README.md#run-a-millie-model-directly).
...
```sh
git clone https://github.com/llmsforall/llama.cpp.git
cd llama.cpp
```
...
Run the following commands from the `llama.cpp` checkout. Install CMake, a C/C++
...
cmake --build build-metal --target llama-server -j 8
```
...
cmake --build build-vulkan --target llama-server -j 8
```
