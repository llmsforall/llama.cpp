...
// UE4M3 scale decode (SYM16 family): raw * 0.5, indices 0x00/0x7F -> 0, bit 7 ignored
// (matches ggml_ue4m3_to_fp32 in ggml-impl.h; all values exact in fp32)
constexpr constant static float kvalues_ue4m3_f[256] = {
...
// Q4_SYM16: block = 32 elems = two 16-elem groups, one UE4M3 scale per group.
// il selects the group: bytes qs[8*il + j] hold elem(16*il + j) in the low
// nibble and elem(16*il + 8 + j) in the high nibble. W = d * (code - 7.5).
template <typename type4x4>
void dequantize_q4_sym16(device const block_q4_sym16 * xb, short il, thread type4x4 & reg) {
    device const uint8_t * qs = xb->qs + 8*il;
    const float d = kvalues_ue4m3_f[xb->d[il]];

    float4x4 reg_f;

    for (int j = 0; j < 8; ++j) {
        reg_f[j/4][j%4]         = d * (((qs[j]     ) & 0x0F) - 7.5f);
        reg_f[(j+8)/4][(j+8)%4] = d * (((qs[j] >> 4) & 0x0F) - 7.5f);
    }

    reg = (type4x4) reg_f;
}

// t4 variant for the mul_mv_ext path: il in 0..7 selects 4 elems (4*il..4*il+3)
template <typename type4>
void dequantize_q4_sym16_t4(device const block_q4_sym16 * xb, short il, thread type4 & reg) {
    const short g = il/4;             // scale group
    const short m = (il%4)%2;         // byte quad within the group
    const short shr = 4*((il%4)/2);   // nibble half

    device const uint8_t * qs = xb->qs + 8*g + 4*m;
    const float d = kvalues_ue4m3_f[xb->d[g]];

    reg[0] = d * (((qs[0] >> shr) & 0x0F) - 7.5f);
    reg[1] = d * (((qs[1] >> shr) & 0x0F) - 7.5f);
    reg[2] = d * (((qs[2] >> shr) & 0x0F) - 7.5f);
    reg[3] = d * (((qs[3] >> shr) & 0x0F) - 7.5f);
}

// Q2_SYM16: block = 16 elems (il is always 0), 4 codes per byte at shifts
// 0/2/4/6, UE4M3 scale. W = d * (code - 1.5).
template <typename type4x4>
void dequantize_q2_sym16(device const block_q2_sym16 * xb, short il, thread type4x4 & reg) {
    const float d = kvalues_ue4m3_f[xb->d];

    float4x4 reg_f;

    for (int j = 0; j < 16; ++j) {
        const int code = (xb->qs[j/4] >> ((j%4)*2)) & 3;
        reg_f[j/4][j%4] = d * (code - 1.5f);
    }

    reg = (type4x4) reg_f;
}

// Q1_SYM32K: 256-elem super-block (nl=16); il selects 16 elems = half of
// sub-block s=il/2. value = bit ? +0.5*d*sc : -0.5*d*sc, bits LSB-first.
template <typename type4x4>
void dequantize_q1_sym32k(device const block_q1_sym32k * xb, short il, thread type4x4 & reg) {
    const short s = il/2;
    const uint8_t scb = xb->scales[s >> 1];
    const float sc = (float)((s & 1) ? (scb >> 4) : (scb & 0x0F));
    const float h = 0.5f * (float)xb->d * sc;

    device const uint8_t * qs = xb->qs + s*4 + 2*(il & 1); // 2 bytes = 16 bits
    const uint16_t bits = (uint16_t)qs[0] | ((uint16_t)qs[1] << 8);

    float4x4 reg_f;
    const float4 hp = float4(h);
    FOR_UNROLL (short r = 0; r < 4; ++r) {
        const ushort b4 = (bits >> (4*r)) & 0xF;
        reg_f[r] = select(-hp, hp, bool4(b4 & 1, b4 & 2, b4 & 4, b4 & 8));
    }
    reg = (type4x4) reg_f;
}

// Q2_SYM32K4: 256-elem super-block (nl=16); il selects 16 elems =
// half of sub-block s=il/2. qs is sequential LSB-first: byte b holds elems
// 4b..4b+3 at shifts 0/2/4/6. value = (code - 1.5) * d * sc[s].
static inline float q2_sym32k_sub_scale(device const block_q2_sym32k4 * xb, short s) {
    const uint8_t scb = xb->scales[s >> 1];
    return (float)((s & 1) ? (scb >> 4) : (scb & 0x0F));
}

template <typename block_t, typename type4x4>
void dequantize_q2_sym32k_common(device const block_t * xb, short il, thread type4x4 & reg) {
    const short s = il/2;
    const float ds = (float)xb->d * q2_sym32k_sub_scale(xb, s);
    device const uint8_t * qs = xb->qs + s*8 + 4*(il & 1); // 4 bytes = 16 codes

    float4x4 reg_f;
    FOR_UNROLL (short r = 0; r < 4; ++r) {
        const uint b = qs[r];
        reg_f[r] = (float4(b & 3u, (b >> 2) & 3u, (b >> 4) & 3u, (b >> 6) & 3u) - 1.5f) * ds;
    }
    reg = (type4x4) reg_f;
}

template <typename type4x4>
void dequantize_q2_sym32k4(device const block_q2_sym32k4 * xb, short il, thread type4x4 & reg) {
    dequantize_q2_sym32k_common<block_q2_sym32k4, type4x4>(xb, il, reg);
}

// QT_MS32K4: ternary (-1/0/+1) mask+sign, 256-elem super-block (nl=16).
// The k-th set mask bit (LSB-first across the WHOLE block) takes sign bit k,
// so any decode needs the prefix popcount of the preceding mask words. Blocks
// may be under sign budget (<128 non-zeros) or fully zero -- the math below
// never assumes exactly-128. Signs are applied position-parallel: element j's
// sign index within the 32-bit window `sw` is popcount of the mask bits below
// j, so sign_j = (sw >> popcount(m & ((1<<j)-1))) & mask_j. All 16 elements
// of a word are independent, with no serial consume chain or lookup table.

// 4 elems (j = 4g..4g+3) of ternary values times ds, from mask word m and
// sign window sw (already shifted so bit 0 = first sign of this word)
template <short G>
static inline float4 tern_val4_pos(ushort m, uint sw, float ds) {
    const uint4 jv = uint4(4*G, 4*G + 1, 4*G + 2, 4*G + 3);
    const uint4 bm = (uint4(1u) << jv) - 1u;              // compile-time
    const uint4 cnt = popcount(uint4(m) & bm);
    const uint4 mb  = (uint4(m)  >> jv)  & 1u;
    const uint4 sb  = (uint4(sw) >> cnt) & mb;
    const float4 pm = select(float4(0.0f), float4( ds), bool4(mb));
    return            select(pm,           float4(-ds), bool4(sb));
}

template <typename type4x4>
void dequantize_qt_ms32k4(device const block_qt_ms32k4 * xb, short il, thread type4x4 & reg) {
    const short s = il/2;
    const uint8_t scb = xb->scales[s >> 1];
    const float sc = (float)((s & 1) ? (scb >> 4) : (scb & 0x0F));
    const float ds = (float)xb->d * sc;

    // block stride is 54 bytes and mask/signs sit at even offsets -> 16-bit
    // loads are aligned; signs are read at fixed in-block offsets only
    device const ushort * m16 = (device const ushort *)(xb->mask);
    device const ushort * s16 = (device const ushort *)(xb->signs);

    // Load the mask in four packed vectors and select preceding words for
    // prefix popcounts, avoiding a thread-local array and memory spills.
    device const packed_ushort4 * pm4 = (device const packed_ushort4 *)(xb->mask);
    const uint4 c0 = popcount(uint4(ushort4(pm4[0])));
    const uint4 c1 = popcount(uint4(ushort4(pm4[1])));
    const uint4 c2 = popcount(uint4(ushort4(pm4[2])));
    const uint4 c3 = popcount(uint4(ushort4(pm4[3])));

    const uint4 ilv = uint4(il);
    const uint4 acc4 = select(uint4(0), c0, uint4( 0,  1,  2,  3) < ilv)
                     + select(uint4(0), c1, uint4( 4,  5,  6,  7) < ilv)
                     + select(uint4(0), c2, uint4( 8,  9, 10, 11) < ilv)
                     + select(uint4(0), c3, uint4(12, 13, 14, 15) < ilv);
    const uint start = acc4.x + acc4.y + acc4.z + acc4.w;

    const ushort m = m16[il]; // il selects exactly one 16-bit mask word

    // 16-bit sign window from bit `start` of the 128-bit sign field; when the
    // window is past the used signs the mask is guaranteed 0 there, so the
    // (clamped) garbage bits are never selected
    const uint i  = start >> 4;
    const uint sh = start & 15;
    uint sw = ((uint)s16[min(i, 7u)] | ((uint)s16[min(i + 1u, 7u)] << 16)) >> sh;

    float4x4 reg_f;
    reg_f[0] = tern_val4_pos<0>(m, sw, ds);
    reg_f[1] = tern_val4_pos<1>(m, sw, ds);
    reg_f[2] = tern_val4_pos<2>(m, sw, ds);
    reg_f[3] = tern_val4_pos<3>(m, sw, ds);
    reg = (type4x4) reg_f;
}

// Q4_SYM16K: 128-elem super-block (nl=8); il selects sub-block s=il directly.
// PAIRWISE nibbles: byte j = elems 2j (lo), 2j+1 (hi). value = (c-7.5)*d*scales[s].
template <typename type4x4>
void dequantize_q4_sym16k(device const block_q4_sym16k * xb, short il, thread type4x4 & reg) {
    const float ds = (float)xb->d * (float)xb->scales[il];
    device const uint8_t * qs = xb->qs + 8*il;

    // Vectorized pairwise unpack: two bytes produce one float4 row,
    // without a scalar per-element loop.
    float4x4 reg_f;
    FOR_UNROLL (short r = 0; r < 4; ++r) {
        const uint8_t b0 = qs[2*r    ];
        const uint8_t b1 = qs[2*r + 1];
        reg_f[r] = (float4(b0 & 0x0F, b0 >> 4, b1 & 0x0F, b1 >> 4) - 7.5f) * ds;
    }
    reg = (type4x4) reg_f;
}

// t4 variant for mul_mv_ext: il in 0..31 selects 4 elems of the 128-elem block
template <typename type4>
void dequantize_q4_sym16k_t4(device const block_q4_sym16k * xb, short il, thread type4 & reg) {
    const short s = il/4;
    const float ds = (float)xb->d * (float)xb->scales[s];
    device const uint8_t * qs = xb->qs + 8*s + 2*(il & 3); // 2 bytes = 4 elems

    reg[0] = ((qs[0] & 0x0F) - 7.5f) * ds;
    reg[1] = ((qs[0] >>   4) - 7.5f) * ds;
    reg[2] = ((qs[1] & 0x0F) - 7.5f) * ds;
    reg[3] = ((qs[1] >>   4) - 7.5f) * ds;
}
...
template [[host_name("kernel_mul_mv_ext_q4_sym16k_f32_r1_2")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<2, block_q4_sym16k, 128, dequantize_q4_sym16k_t4>;
template [[host_name("kernel_mul_mv_ext_q4_sym16k_f32_r1_3")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<3, block_q4_sym16k, 128, dequantize_q4_sym16k_t4>;
template [[host_name("kernel_mul_mv_ext_q4_sym16k_f32_r1_4")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<4, block_q4_sym16k, 128, dequantize_q4_sym16k_t4>;
template [[host_name("kernel_mul_mv_ext_q4_sym16k_f32_r1_5")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<5, block_q4_sym16k, 128, dequantize_q4_sym16k_t4>;
template [[host_name("kernel_mul_mv_ext_q4_sym16k_f32_r1_8")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<8, block_q4_sym16k, 128, dequantize_q4_sym16k_t4>;
template [[host_name("kernel_mul_mv_ext_q4_sym16_f32_r1_2")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<2, block_q4_sym16, 32, dequantize_q4_sym16_t4>;
template [[host_name("kernel_mul_mv_ext_q4_sym16_f32_r1_3")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<3, block_q4_sym16, 32, dequantize_q4_sym16_t4>;
template [[host_name("kernel_mul_mv_ext_q4_sym16_f32_r1_4")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<4, block_q4_sym16, 32, dequantize_q4_sym16_t4>;
template [[host_name("kernel_mul_mv_ext_q4_sym16_f32_r1_5")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<5, block_q4_sym16, 32, dequantize_q4_sym16_t4>;
...
#define NS20 (FC_flash_attn_ext_vec_ns20)
    ...
    const ushort iq2 = tgpig[1];
    const ushort iq1 = tgpig[0]*Q; // first query row of this threadgroup's group of Q
  ...
  //const short T = Q*PK + NSG*Q*SH; // shared memory size in (half)

    // Q > 1 (multi-query): regions are repeated per query row; layout keeps the
    // Q==1 case byte-identical to the original single-query kernel.
  //threadgroup q_t   * sq  = (threadgroup q_t   *) (shmem_f16 +                      0*PK); // holds the query data (Q rows)
    threadgroup q4_t  * sq4 = (threadgroup q4_t  *) (shmem_f16 +                      0*PK); // same as above but in q4_t
    threadgroup s_t   * ss  = (threadgroup s_t   *) (shmem_f16 +   sgitg*Q*SH       + NSG*Q*PK); // scratch buffer for attention (Q rows)
    threadgroup s4_t  * ss4 = (threadgroup s4_t  *) (shmem_f16 +   sgitg*Q*SH       + NSG*Q*PK); // same as above but in s4_t
    threadgroup half  * sm  = (threadgroup half  *) (shmem_f16 +   sgitg*Q*SH + Q*2*C + NSG*Q*PK); // scratch buffer for mask (Q rows)
    threadgroup o4_t  * so4 = (threadgroup o4_t  *) (shmem_f16 + 2*sgitg*Q*PV       + NSG*Q*PK + NSG*Q*SH); // scratch buffer for the results (Q rows)
    ...
    // load heads from Q to shared memory (Q rows)
    FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
        device const float4 * q4 = (device const float4 *) ((device const char *) q + jq*args.nb01);

        if (iq1 + jq < args.ne01) {
            for (short i = tiisg; i < PK4; i += NW) {
                if (i < DK4) {
                    sq4[jq*PK4 + i] = (q4_t) q4[i];
                } else {
                    sq4[jq*PK4 + i] = (q4_t) 0.0f;
                }
            }
        } else {
            for (short i = tiisg; i < PK4; i += NW) {
    ...
    FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
        for (short i = 0; i < DV4/NL; ++i) {
            so4[jq*PV4 + i*NL] = (o4_t) 0.0f;
        }
    }

    // zero out shared memory SH (Q rows)
    for (short i = tiisg; i < Q*SH/4; i += NW) {
        ss4[i] = (s4_t) 0.0f;
    }
...

    {
        ...
        // pointers to the mask (one row per query)
        device const half * pm[Q];
        FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
            pm[jq] = (device const half *) (mask + (iq1 + jq)*args.nb31 + (iq2%args.ne32)*args.nb32 + (iq3%args.ne33)*args.nb33);
        }
        ...
        for (int ic0 = iwg*NSG + sgitg; ; ic0 += NWG*NSG) {
            ...
            if (FC_flash_attn_ext_vec_has_kvpad && ic + C > args.ne11) {
                ...
                if (!FC_flash_attn_ext_vec_has_mask) {
                    if (ic + tiisg >= args.ne11) {
                        FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
                            sm[jq*C + tiisg] = -MAXHALF;
                        }
                    }
                } else {
                    FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
                        pm[jq] = (device const half *) (mask) +
                            (iq1 + jq)*C +
                            (iq2%args.ne32)*(C*args.ne31) +
                            (iq3%args.ne33)*(C*args.ne31*args.ne32);
                    }
            ...
            if (FC_flash_attn_ext_vec_has_mask) {
                FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
                    sm[jq*C + tiisg] = pm[jq][ic + tiisg];
                }
            }

            // skip blocks that are -INF for ALL query rows
            {
                half smax = sm[tiisg];
                FOR_UNROLL (short jq = 1; jq < Q; ++jq) {
                    smax = max(smax, sm[jq*C + tiisg]);
                }
                if (simd_max(smax) <= -MAXHALF) {
                    continue;
                }
            }
            ...
            // Q*K^T
            {
                ...
                // each K value is loaded once and dotted against all Q query rows
                FOR_UNROLL (short cc = 0; cc < C/NE; ++cc) {
                    if (is_same<kd4_t, k4_t>::value) {
                        FOR_UNROLL (short ii = 0; ii < DK4/NL; ++ii) {
                            const float4 kk = (float4) pk4[cc*NE*NS10/4 + ii*NL];
                            FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
                                mqk[jq][cc] += dot(kk, (float4) pq4[jq*PK4 + ii*NL]);
                            }
                        }
                    } else {
                        ...
                        FOR_UNROLL (short ii = 0; ii < DK4/NL; ++ii) {
                            ...
                            const float4 kk = (float4) mk;
                            FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
                                mqk[jq][cc] += dot(kk, (float4) sq4[jq*PK4 + i]);
                            }
                        }
                    }
                }
                ...
                FOR_UNROLL (short cc = 0; cc < C/NE; ++cc) {
                    if (NE == 1) {
                        ...
                        // [24 .. 31] -> [24]
                        if (NE <= 1) {
                            mqk[jq][cc] += simd_shuffle_down(mqk[jq][cc], 16);
                        }
                        if (NE <= 2) {
                            mqk[jq][cc] += simd_shuffle_down(mqk[jq][cc],  8);
                        }
                        if (NE <= 4) {
                            mqk[jq][cc] += simd_shuffle_down(mqk[jq][cc],  4);
                        }
                        if (NE <= 8) {
                            mqk[jq][cc] += simd_shuffle_down(mqk[jq][cc],  2);
                        }
                        if (NE <= 16) {
                            mqk[jq][cc] += simd_shuffle_down(mqk[jq][cc],  1);
                        }

                        // broadcast
                        mqk[jq][cc] = simd_shuffle(mqk[jq][cc], NL*ty);
                    }
                }
                }
                ...
                if (FC_flash_attn_ext_vec_has_mask &&
                   ...
                   !FC_flash_attn_ext_vec_has_bias) {
                    ss[jq*C + NE*tx + ty] = fma(mqk[jq][tx], args.scale, (qk_t) sm[jq*C + NE*tx + ty]);
                } else {
                    mqk[jq][tx] *= args.scale;

                    if (FC_flash_attn_ext_vec_has_scap) {
                        mqk[jq][tx] = args.logit_softcap*precise::tanh(mqk[jq][tx]);
                    }

                    if (FC_flash_attn_ext_vec_has_bias) {
                        mqk[jq][tx] += (qk_t) sm[jq*C + NE*tx + ty]*slope;
                    } else {
                        mqk[jq][tx] += (qk_t) sm[jq*C + NE*tx + ty];
                    }

                    ss[jq*C + NE*tx + ty] = mqk[jq][tx];
                }
                }
            }
            ...
            FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
                const float m = M[jq];
                const float s = ss[jq*C + tiisg];

                M[jq] = simd_max(max(M[jq], s));

                const float ms = exp(m - M[jq]);
                const float vs = exp(s - M[jq]);

                S[jq] = S[jq]*ms + simd_sum(vs);
            ...
            // O = O + (Q*K^T)*V (first cut: V is re-read per query row; K sharing
            // above is the dominant win and this keeps register pressure safe)
            FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
                o4_t lo[DV4/NL];
                FOR_UNROLL (short ii = 0; ii < DV4/NL; ++ii) {
                    lo[ii] = 0.0f;
                ...
                if (is_same<vd4_t, v4_t>::value) {
                    ...
                    FOR_UNROLL (short cc = 0; cc < C/NE; ++cc) {
                        ...
                        FOR_UNROLL (short ii = 0; ii < DV4/NL; ++ii) {
                            ...
                            lo[ii] += o4_t(float4(mv)*float4(ss[jq*C + NE*cc + ty]));
                        }
                    }
                }
    ...
    // parallel reduce (per query row)
    for (short r = NSG/2; r > 0; r >>= 1) {
        if (sgitg < r) {
            FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
                const float S0 = ss[jq*C +                0];
                const float S1 = ss[jq*C + r*(Q*SH/2) +   0];

                const float M0 = ss[jq*C +                1];
                const float M1 = ss[jq*C + r*(Q*SH/2) +   1];
                ...
                for (short i = tiisg; i < DV4; i += NW) {
                    so4[jq*PV4 + i] = so4[jq*PV4 + i]*ms0 + so4[jq*PV4 + i + r*(Q*PV4)]*ms1;
                }
            }
        }
    ...
    // final rescale with 1/S and store to global memory (per query row)
    if (sgitg == 0) {
        ...
        FOR_UNROLL (short jq = 0; jq < Q; ++jq) {
            ...
            const int64_t rid = iq3*args.ne2*args.ne1 + iq2 + (iq1 + jq)*args.ne1;

            const float S = NWG == 1 ? (ss[jq*C + 0] == 0.0f ? 0.0f : 1.0f/ss[jq*C + 0]) : 1.0f;

            // interleave the workgroup data
            for (short i = tiisg; i < DV4; i += NW) {
                dst4[rid*DV4*NWG + NWG*i + iwg] = (float4) so4[jq*PV4 + i]*S;
            }

            // store S and M
            if (NWG > 1) {
                if (tiisg == 0) {
                    dst1[rid*(2*NWG) + 2*iwg + 0] = ss[jq*C + 0];
                    dst1[rid*(2*NWG) + 2*iwg + 1] = ss[jq*C + 1];
                }
            }
        }
    }
...
// multi-query variant (Q=4): shared-KV multi-stream decode (K read once per 4 streams)
template [[host_name("kernel_flash_attn_ext_vec_mq4_f16_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, half4,     1, dequantize_f16_t4,  half4,       1, dequantize_f16_t4,  256, 256, 1, 4>;
#if defined(GGML_METAL_HAS_BF16)
...
// Q4_SYM16 mat-vec: same structure as mxfp4 (split-nibble 8-byte chunks, shmem
// LUT for the 4-bit codes), but the LUT is the uniform (code - 7.5) ladder and
// the UE4M3 scale is per 16-elem group (two per block), applied per chunk.
template<short NR0, typename args_t>
void kernel_mul_mv_q4_sym16_f32_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    device const block_q4_sym16 * x = (device const block_q4_sym16 *) (src0 + offset0);
    device const float          * y = (device const float          *) (src1 + offset1);

    const int nb   = args.ne00/QK4_SYM16;
    const int ns01 = args.nb01/args.nb00;

    const short ix = tiisg/2;  // 0...15
    const short it = tiisg%2;  // group within the block

    shmem_f32[tiisg] = (float)(tiisg%16) - 7.5f;
    threadgroup_barrier(mem_flags::mem_threadgroup);

    float4 yl[4];
    float sumf[NR0]={0.f};

    device const float * yb = y + ix*QK4_SYM16 + it*16;

    for (int ib = ix; ib < nb && ib < ns01; ib += 16) {
        device const float4 * y4 = (device const float4 *) yb;

        yl[0] = y4[0]; // elems 16*it + 0..3  (low  nibbles of bytes 0..3)
        yl[1] = y4[2]; // elems 16*it + 8..11 (high nibbles of bytes 0..3)
        yl[2] = y4[1]; // elems 16*it + 4..7  (low  nibbles of bytes 4..7)
        yl[3] = y4[3]; // elems 16*it + 12..15(high nibbles of bytes 4..7)

        FOR_UNROLL (short row = 0; row < NR0; row++) {
            device const block_q4_sym16 & xb = x[row*ns01 + ib];
            device const uint8_t        * q2 = (device const uint8_t *)(xb.qs + 8*it);
            ...
            sumf[row] += kvalues_ue4m3_f[xb.d[it]] * ((acc1[0] + acc1[1]) + (acc1[2] + acc1[3]));
        }
...
[[host_name("kernel_mul_mv_q4_sym16_f32")]]
kernel void kernel_mul_mv_q4_sym16_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    kernel_mul_mv_q4_sym16_f32_impl<N_R0_Q4_SYM16, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}

// Q2_SYM16 mat-vec: 16-elem blocks (5 bytes), one block per thread per
// iteration, codes decoded arithmetically (levels are the uniform
// (code - 1.5) ladder). No shared memory.
template<short NR0, typename args_t>
void kernel_mul_mv_q2_sym16_f32_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    device const block_q2_sym16 * x = (device const block_q2_sym16 *) (src0 + offset0);
    device const float          * y = (device const float          *) (src1 + offset1);

    const int nb   = args.ne00/QK2_SYM16;
    const int ns01 = args.nb01/args.nb00;

    const short ix = tiisg;  // 0...31

    float sumf[NR0]={0.f};

    device const float * yb = y + ix*QK2_SYM16;

    for (int ib = ix; ib < nb && ib < ns01; ib += 32) {
        device const float4 * y4 = (device const float4 *) yb;

        const float4 yl0 = y4[0];
        const float4 yl1 = y4[1];
        const float4 yl2 = y4[2];
        const float4 yl3 = y4[3];

        FOR_UNROLL (short row = 0; row < NR0; row++) {
            device const block_q2_sym16 & xb = x[row*ns01 + ib];

            const uint8_t b0 = xb.qs[0];
            const uint8_t b1 = xb.qs[1];
            const uint8_t b2 = xb.qs[2];
            const uint8_t b3 = xb.qs[3];

            float4 acc = yl0*(float4(b0 & 3, (b0 >> 2) & 3, (b0 >> 4) & 3, (b0 >> 6) & 3) - 1.5f)
                       + yl1*(float4(b1 & 3, (b1 >> 2) & 3, (b1 >> 4) & 3, (b1 >> 6) & 3) - 1.5f)
                       + yl2*(float4(b2 & 3, (b2 >> 2) & 3, (b2 >> 4) & 3, (b2 >> 6) & 3) - 1.5f)
                       + yl3*(float4(b3 & 3, (b3 >> 2) & 3, (b3 >> 4) & 3, (b3 >> 6) & 3) - 1.5f);

            sumf[row] += kvalues_ue4m3_f[xb.d] * ((acc[0] + acc[1]) + (acc[2] + acc[3]));
        }
...
// Q1_SYM32K mat-vec: each thread owns one 32-elem sub-block per iteration
// (ix = block, it = sub-block). Sign-select against y, scale 0.5*d*sc hoisted.
template<short NR0, typename args_t>
void kernel_mul_mv_q1_sym32k_f32_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    device const block_q1_sym32k * x = (device const block_q1_sym32k *) (src0 + offset0);
    device const float           * y = (device const float           *) (src1 + offset1);

    const int nb   = args.ne00/QK1_SYM32K;
    const int ns01 = args.nb01/args.nb00;

    const short ix = tiisg/8; // 0..3: block in flight
    const short it = tiisg%8; // sub-block within the block

    float sumf[NR0]={0.f};

    device const float * yb = y + ix*QK1_SYM32K + it*QK1_SYM32K_SUB;

    for (int ib = ix; ib < nb && ib < ns01; ib += 4) {
        device const float4 * y4 = (device const float4 *) yb;

        FOR_UNROLL (short row = 0; row < NR0; row++) {
            device const block_q1_sym32k & xb = x[row*ns01 + ib];

            const uint8_t scb = xb.scales[it >> 1];
            const float sc = (float)((it & 1) ? (scb >> 4) : (scb & 0x0F));

            device const uint8_t * qs = (device const uint8_t *)(xb.qs + it*4);
            const uint32_t bits = (uint32_t)qs[0] | ((uint32_t)qs[1] << 8) | ((uint32_t)qs[2] << 16) | ((uint32_t)qs[3] << 24);

            float4 acc = float4(0.f);
            FOR_UNROLL (short g = 0; g < 8; ++g) {
                const uint b4 = (bits >> (4*g)) & 0xF;
                const float4 sgn = float4(
                    (b4 & 1u) ? 1.f : -1.f,
                    (b4 & 2u) ? 1.f : -1.f,
                    (b4 & 4u) ? 1.f : -1.f,
                    (b4 & 8u) ? 1.f : -1.f);
            ...
            sumf[row] += 0.5f * (float)xb.d * sc * ((acc[0] + acc[1]) + (acc[2] + acc[3]));
        }
...
[[host_name("kernel_mul_mv_q1_sym32k_f32")]]
kernel void kernel_mul_mv_q1_sym32k_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    kernel_mul_mv_q1_sym32k_f32_impl<N_R0_Q1_SYM32K, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}

// Q2_SYM32K4 mat-vec: each thread owns one 32-elem sub-block per
// iteration (8 sub-blocks per 256-elem block -> 8 threads per block, 4 blocks
// in flight per simdgroup), mirroring the Q1_SYM32K kernel.
template<typename block_t, short NR0, typename args_t>
void kernel_mul_mv_q2_sym32k_common_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    device const block_t * x = (device const block_t *) (src0 + offset0);
    device const float   * y = (device const float   *) (src1 + offset1);

    const int nb   = args.ne00/QK2_SYM32K;
    const int ns01 = args.nb01/args.nb00;
    ...
    device const float * yb = y + ix*QK2_SYM32K + it*QK2_SYM32K_SUB;

    for (int ib = ix; ib < nb && ib < ns01; ib += 4) {
        device const float4 * y4 = (device const float4 *) yb;

        FOR_UNROLL (short row = 0; row < NR0; row++) {
            device const block_t * xb = x + row*ns01 + ib;

            const float sc = q2_sym32k_sub_scale(xb, it);
            device const uint8_t * qs = xb->qs + it*8; // 8 bytes = 32 codes

            float4 acc = float4(0.f);
            FOR_UNROLL (short g = 0; g < 8; ++g) {
                const uint b = qs[g];
                const float4 c = float4(b & 3u, (b >> 2) & 3u, (b >> 4) & 3u, (b >> 6) & 3u) - 1.5f;
                acc += c * y4[g];
            }

            sumf[row] += (float)xb->d * sc * ((acc[0] + acc[1]) + (acc[2] + acc[3]));
        }
...
template<short NR0, typename args_t>
void kernel_mul_mv_q2_sym32k4_f32_impl(
    ...
    kernel_mul_mv_q2_sym32k_common_impl<block_q2_sym32k4, NR0, args_t>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}

[[host_name("kernel_mul_mv_q2_sym32k4_f32")]]
kernel void kernel_mul_mv_q2_sym32k4_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    kernel_mul_mv_q2_sym32k4_f32_impl<N_R0_Q2_SYM32K4, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}

// QT_MS32K4 mat-vec: same shape as the q2_sym32k kernel (one thread owns one
// 32-elem sub-block, 8 threads per 256-elem block, 4 blocks in flight per
// simdgroup). Decode per sub-block: prefix popcount of the preceding mask
// words gives the running sign start (budget is per BLOCK, not sub-block),
// a 32-bit sign window is taken from fixed in-block 16-bit loads, and the
// signs are scattered onto the mask bits one nibble at a time.
template<short NR0, typename args_t>
void kernel_mul_mv_qt_ms32k4_f32_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    device const block_qt_ms32k4 * x = (device const block_qt_ms32k4 *) (src0 + offset0);
    device const float           * y = (device const float           *) (src1 + offset1);

    const int nb   = args.ne00/QKT_MS32K;
    const int ns01 = args.nb01/args.nb00;
    ...
    device const float * yb = y + ix*QKT_MS32K + it*QKT_MS32K_SUB;

    // The loop is UNIFORM across the simdgroup (bound doesn't depend on ix)
    // so the segmented prefix sum below sees all 32 lanes every iteration;
    // lanes whose block is past the end contribute own=0. The 8 lanes of a
    // block are contiguous (tiisg = ix*8 + it), so the running sign start is
    // pre(lane) - pre(segment base) -- each thread then loads only its OWN
    // two mask words instead of re-reading every preceding word.
    const int nbmin = min(nb, ns01);

    for (int ib0 = 0; ib0 < nbmin; ib0 += 4) {
        const int  ib     = ib0 + ix;
        const bool active = ib < nbmin;

        device const float4 * y4 = (device const float4 *) yb;

        FOR_UNROLL (short row = 0; row < NR0; row++) {
            device const block_qt_ms32k4 * xb = x + row*ns01 + ib;
            ...
            const uint own   = popcount((uint)m0) + popcount((uint)m1);
            const uint pre   = simd_prefix_exclusive_sum(own);
            const uint start = pre - simd_shuffle(pre, (ushort)(ix*8));
            ...
            if (active) {
                ...
                // up to 32 sign bits from bit `start`; fixed-offset loads only
                const uint i  = start >> 4;
                const uint sh = start & 15;
                const uint lo = (uint)s16[min(i,      7u)]
                             | ((uint)s16[min(i + 1u, 7u)] << 16);
                const uint hi =  (uint)s16[min(i + 2u, 7u)];
                uint sw = sh == 0 ? lo : ((lo >> sh) | (hi << (32 - sh)));

                const uint sw1 = sw >> popcount((uint)m0);

                float4 acc = float4(0.f);
                acc += tern_val4_pos<0>(m0, sw,  1.0f) * y4[0];
                acc += tern_val4_pos<1>(m0, sw,  1.0f) * y4[1];
                acc += tern_val4_pos<2>(m0, sw,  1.0f) * y4[2];
                acc += tern_val4_pos<3>(m0, sw,  1.0f) * y4[3];
                acc += tern_val4_pos<0>(m1, sw1, 1.0f) * y4[4];
                acc += tern_val4_pos<1>(m1, sw1, 1.0f) * y4[5];
                acc += tern_val4_pos<2>(m1, sw1, 1.0f) * y4[6];
                acc += tern_val4_pos<3>(m1, sw1, 1.0f) * y4[7];
...
[[host_name("kernel_mul_mv_qt_ms32k4_f32")]]
kernel void kernel_mul_mv_qt_ms32k4_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    kernel_mul_mv_qt_ms32k4_f32_impl<N_R0_QT_MS32K4, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}

// Q4_SYM16K mat-vec: each thread owns one 32-elem chunk (2 sub-blocks) of a
// 128-elem block. Pairwise nibbles pair naturally with float2 loads of y.
template<short NR0, typename args_t>
void kernel_mul_mv_q4_sym16k_f32_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    device const block_q4_sym16k * x = (device const block_q4_sym16k *) (src0 + offset0);
    device const float           * y = (device const float           *) (src1 + offset1);

    const int nb   = args.ne00/QK4_SYM16K;
    const int ns01 = args.nb01/args.nb00;

    const short ix = tiisg/4; // 0..7: block in flight
    const short it = tiisg%4; // 32-elem chunk = sub-blocks 2*it, 2*it+1

    float sumf[NR0]={0.f};

    device const float * yb = y + ix*QK4_SYM16K + it*2*QK4_SYM16K_SUB;

    for (int ib = ix; ib < nb && ib < ns01; ib += 8) {
        device const float2 * y2 = (device const float2 *) yb;

        FOR_UNROLL (short row = 0; row < NR0; row++) {
            device const block_q4_sym16k & xb = x[row*ns01 + ib];
            device const uint8_t * qs = (device const uint8_t *)(xb.qs + 16*it);

            float2 acc0 = float2(0.f);
            float2 acc1 = float2(0.f);
            FOR_UNROLL (short j = 0; j < 8; ++j) {
                const uint8_t b0 = qs[j];
                const uint8_t b1 = qs[j + 8];
                acc0 += float2((b0 & 0x0F) - 7.5f, (b0 >> 4) - 7.5f) * y2[j];
                acc1 += float2((b1 & 0x0F) - 7.5f, (b1 >> 4) - 7.5f) * y2[j + 8];
            }

            const float d = (float)xb.d;
            sumf[row] += d * ((float)xb.scales[2*it    ] * (acc0[0] + acc0[1]) +
                              (float)xb.scales[2*it + 1] * (acc1[0] + acc1[1]));
        }
...
[[host_name("kernel_mul_mv_q4_sym16k_f32")]]
kernel void kernel_mul_mv_q4_sym16k_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    kernel_mul_mv_q4_sym16k_f32_impl<N_R0_Q4_SYM16K, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}

[[host_name("kernel_mul_mv_q2_sym16_f32")]]
kernel void kernel_mul_mv_q2_sym16_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    kernel_mul_mv_q2_sym16_f32_impl<N_R0_Q2_SYM16, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}
...
// QT_MS32K4 mm_id: clone of kernel_mul_mm_id where the running sign index is
// carried in a REGISTER across K iterations. The two threads that load a row
// (il0 = 0/1) walk the same 256-elem block for 8 consecutive NK=32
// iterations (il += 2), so instead of the generic dequantize path's
// per-chunk prefix scan over the whole mask (the dominant mm cost for this
// type), the sign start advances with one extra ushort load + two popcounts
// per iteration and resets when the walk enters a new block.
template<typename T1_t, typename T1_2x4_t>
kernel void kernel_mul_mm_id_qt_ms32k4(
        constant ggml_metal_kargs_mul_mm_id & args,
        device const char * src0,
        device const char * src1,
        device const char * htpe,
...
#ifdef GGML_METAL_HAS_TENSOR
    threadgroup float * sc_tg = (threadgroup float *)(shmem);
#endif
    ...
    device const block_qt_ms32k4 * x = (device const block_qt_ms32k4 *)(src0 + args.nb01*(r0 + lr0) + offset0);

    const short iy = 8*(tiitg % NL1);

    device const T1_t * y = (device const T1_t *)(src1
        + args.nb13*i13
        + args.nb12*i12
        + args.nb11*i11
        + args.nb10*iy);
    ...
    uint run = 0; // sign index of this thread's current chunk within its block

    for (int loop_k = 0; loop_k < args.ne00; loop_k += NK) {
        // inline stateful decode of chunk il (replaces dequantize_func)
        half4x4 temp_a;
        {
            device const ushort * m16 = (device const ushort *)(x->mask);
            device const ushort * s16 = (device const ushort *)(x->signs);

            const ushort m = m16[il];

            if (il == il0) { // first chunk of a new block for this thread
                run = il0 == 0 ? 0u : (uint)popcount((uint)m16[0]);
            }

            const short sblk = il/2;
            const uint8_t scb = x->scales[sblk >> 1];
            const float sc = (float)((sblk & 1) ? (scb >> 4) : (scb & 0x0F));
            const float ds = (float)x->d * sc;

            const uint iw = run >> 4;
            const uint sh = run & 15;
            const uint sw = ((uint)s16[min(iw, 7u)] | ((uint)s16[min(iw + 1u, 7u)] << 16)) >> sh;

            float4x4 fa;
            fa[0] = tern_val4_pos<0>(m, sw, ds);
            fa[1] = tern_val4_pos<1>(m, sw, ds);
            fa[2] = tern_val4_pos<2>(m, sw, ds);
            fa[3] = tern_val4_pos<3>(m, sw, ds);

            if (il + 2 < nl) { // advance to this thread's next chunk in the block
                run += (uint)popcount((uint)m) + (uint)popcount((uint)m16[il + 1]);
            }
        ...
        if (FC_mul_mm_bc_inp) {
            for (short i = 0; i < 8; ++i) {
                ...
                *(sb + 64*ib + 8*ly + lx) = loop_k + iy + i < args.ne00 ? (S1) *((device T1_t *) y + i) : 0;
            }
        } else {
            const short sx = (tiitg%NL1);
            const short sy = (tiitg/NL1)/8;
            ...
            *(threadgroup S1_2x4 *)(sb + 64*ib + 8*ly) = (S1_2x4)(*((device T1_2x4_t *) y));
        }
#else
        FOR_UNROLL (short i = 0; i < 16; i++) {
            const short sx = 2*il0 + i/8;
        ...
        if (FC_mul_mm_bc_inp) {
            for (short i = 0; i < 8; ++i) {
                ...
                *(sb + NK*(8*sy + ly) + 8*sx + lx) = loop_k + iy + i < args.ne00 ? (S1) *((device T1_t *) y + i) : 0;
            }
        } else {
            const short sx = (tiitg%NL1);
            const short sy = (tiitg/NL1)/8;
            ...
            *(threadgroup S1_2x4 *)(sb + NK*(8*sy + ly) + 8*sx) = (S1_2x4)(*((device T1_2x4_t *) y));
        }
#endif
...
#ifdef GGML_METAL_HAS_TENSOR
    auto tC = tensor<threadgroup float, dextents<int32_t, 2>, tensor_inline>(sc_tg, dextents<int32_t, 2>(NR0, NR1));
    cT.store(tC);
#else
    threadgroup float * temp_str = ((threadgroup float *) shmem) + 32*(sgitg&1) + (16*(sgitg >> 1))*NR0;
...
typedef decltype(kernel_mul_mm_id_qt_ms32k4<float, float2x4>) mul_mm_id_qt_ms32k4_t;
...
template [[host_name("kernel_get_rows_q4_sym16")]] kernel get_rows_q_t kernel_get_rows_q<block_q4_sym16, 2, dequantize_q4_sym16>;
template [[host_name("kernel_get_rows_q1_sym32k")]] kernel get_rows_q_t kernel_get_rows_q<block_q1_sym32k, 16, dequantize_q1_sym32k>;
template [[host_name("kernel_get_rows_q2_sym32k4")]] kernel get_rows_q_t kernel_get_rows_q<block_q2_sym32k4, 16, dequantize_q2_sym32k4>;
template [[host_name("kernel_get_rows_qt_ms32k4")]]  kernel get_rows_q_t kernel_get_rows_q<block_qt_ms32k4,  16, dequantize_qt_ms32k4>;
template [[host_name("kernel_get_rows_q4_sym16k")]] kernel get_rows_q_t kernel_get_rows_q<block_q4_sym16k, 8, dequantize_q4_sym16k>;
template [[host_name("kernel_get_rows_q2_sym16")]] kernel get_rows_q_t kernel_get_rows_q<block_q2_sym16, 1, dequantize_q2_sym16>;
...
template [[host_name("kernel_mul_mm_q4_sym16_f32")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16, 2,    dequantize_q4_sym16, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_q2_sym16_f32")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_sym16, 1,    dequantize_q2_sym16, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_q1_sym32k_f32")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q1_sym32k, 16,  dequantize_q1_sym32k, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_q2_sym32k4_f32")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_sym32k4, 16,  dequantize_q2_sym32k4, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_qt_ms32k4_f32")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_qt_ms32k4, 16,  dequantize_qt_ms32k4, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_q4_sym16k_f32")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16k, 8,   dequantize_q4_sym16k, float, float4x4,  float, float2x4>;
...
template [[host_name("kernel_mul_mm_q4_sym16_f16")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16, 2,    dequantize_q4_sym16, float, float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_q2_sym16_f16")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_sym16, 1,    dequantize_q2_sym16, float, float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_q1_sym32k_f16")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q1_sym32k, 16,  dequantize_q1_sym32k, float, float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_q2_sym32k4_f16")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_sym32k4, 16,  dequantize_q2_sym32k4, float, float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_qt_ms32k4_f16")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_qt_ms32k4, 16,  dequantize_qt_ms32k4, float, float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_q4_sym16k_f16")]] kernel mul_mm_t kernel_mul_mm<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16k, 8,   dequantize_q4_sym16k, float, float4x4,  half, half2x4>;
...
template [[host_name("kernel_mul_mm_id_q4_sym16_f32")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16, 2,    dequantize_q4_sym16, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_id_q2_sym16_f32")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_sym16, 1,    dequantize_q2_sym16, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_id_q1_sym32k_f32")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q1_sym32k, 16,  dequantize_q1_sym32k, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_id_q2_sym32k4_f32")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_sym32k4, 16,  dequantize_q2_sym32k4, float, float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_id_qt_ms32k4_f32")]] kernel mul_mm_id_qt_ms32k4_t kernel_mul_mm_id_qt_ms32k4<float, float2x4>;
template [[host_name("kernel_mul_mm_id_q4_sym16k_f32")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16k, 8,   dequantize_q4_sym16k, float, float4x4,  float, float2x4>;
...
template [[host_name("kernel_mul_mm_id_q4_sym16_f16")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16, 2,    dequantize_q4_sym16, float, float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_id_q2_sym16_f16")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_sym16, 1,    dequantize_q2_sym16, float, float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_id_q1_sym32k_f16")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q1_sym32k, 16,  dequantize_q1_sym32k, float, float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_id_q2_sym32k4_f16")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q2_sym32k4, 16,  dequantize_q2_sym32k4, float, float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_id_qt_ms32k4_f16")]] kernel mul_mm_id_qt_ms32k4_t kernel_mul_mm_id_qt_ms32k4<half, half2x4>;
template [[host_name("kernel_mul_mm_id_q4_sym16k_f16")]] kernel mul_mm_id kernel_mul_mm_id<half,  half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_q4_sym16k, 8,   dequantize_q4_sym16k, float, float4x4,  half, half2x4>;
...
template [[host_name("kernel_mul_mv_id_q4_sym16_f32")]] kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_q4_sym16_f32_impl<N_R0_Q4_SYM16>>>;
template [[host_name("kernel_mul_mv_id_q2_sym16_f32")]] kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_q2_sym16_f32_impl<N_R0_Q2_SYM16>>>;
template [[host_name("kernel_mul_mv_id_q1_sym32k_f32")]] kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_q1_sym32k_f32_impl<N_R0_Q1_SYM32K>>>;
template [[host_name("kernel_mul_mv_id_q2_sym32k4_f32")]] kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_q2_sym32k4_f32_impl<N_R0_Q2_SYM32K4>>>;
template [[host_name("kernel_mul_mv_id_qt_ms32k4_f32")]] kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_qt_ms32k4_f32_impl<N_R0_QT_MS32K4>>>;
template [[host_name("kernel_mul_mv_id_q4_sym16k_f32")]] kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_q4_sym16k_f32_impl<N_R0_Q4_SYM16K>>>;
