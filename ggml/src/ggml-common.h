...
// 1.5 bpw: 1-bit affine (scale + min) codes, group=32, UE4M3 (fp8) scale and min.
// x ~= scale*q + min, q in {0,1}. scale/min stored as ggml_fp32_to_ue4m3 codes.
#define QK1_AFFINE 32
typedef struct {
    uint8_t d;              // UE4M3 scale
    uint8_t m;              // UE4M3 |min| (min is always <= 0, sign implied)
    uint8_t qs[QK1_AFFINE / 8]; // 32 packed 1-bit codes
} block_q1_affine;
static_assert(sizeof(block_q1_affine) == 2 + QK1_AFFINE / 8, "wrong q1_affine block size/padding");

// Symmetric-only (no min), 1-bit, UE4M3 (fp8) scale. Two group sizes for two bpw targets:
// group=16 -> 1 + 8/16 = 1.5 bpw; group=64 -> 1 + 8/64 = 1.125 bpw.
#define QK1_SYM16 16
typedef struct {
    uint8_t d;                 // UE4M3 scale
    uint8_t qs[QK1_SYM16 / 8]; // 16 packed 1-bit codes
} block_q1_sym16;
static_assert(sizeof(block_q1_sym16) == 1 + QK1_SYM16 / 8, "wrong q1_sym16 block size/padding");

#define QK1_SYM64 64
typedef struct {
    uint8_t d;                 // UE4M3 scale
    uint8_t qs[QK1_SYM64 / 8]; // 64 packed 1-bit codes
} block_q1_sym64;
static_assert(sizeof(block_q1_sym64) == 1 + QK1_SYM64 / 8, "wrong q1_sym64 block size/padding");

// Symmetric-only (no min), 2-bit (4-level, {-1.5,-0.5,+0.5,+1.5}*scale), group=16,
// UE4M3 (fp8) scale. 2 + 8/16 = 2.5 bpw. Meant as a fair, matched-group flat
// comparison against Q2_K (2.625 bpw).
#define QK2_SYM16 16
#define QR2_SYM16 4              // 2-bit codes: 4 per byte
#define QI2_SYM16 1              // 32-bit words of packed codes per block (4 bytes)
typedef struct {
    uint8_t d;                 // UE4M3 scale
    uint8_t qs[QK2_SYM16 / 4]; // 16 packed 2-bit codes
} block_q2_sym16;
static_assert(sizeof(block_q2_sym16) == 1 + QK2_SYM16 / 4, "wrong q2_sym16 block size/padding");

// Q1_SYM32K: 1-bit symmetric K-scale type.
// 8 sub-blocks of 32; effective scale of sub s = d * scales4[s] (unsigned int4,
// even index in the low nibble); value = bit ? +0.5*d*sc : -0.5*d*sc.
#define QK1_SYM32K_SUB   32
#define QK1_SYM32K_SUPER 8
#define QK1_SYM32K (QK1_SYM32K_SUB * QK1_SYM32K_SUPER) // 256 elements
typedef struct {
    ggml_half d;                             // fp16 super-scale
    uint8_t   scales[QK1_SYM32K_SUPER / 2];  // 8 x 4-bit unsigned sub-scales
    uint8_t   qs[QK1_SYM32K / 8];            // 1 bit/elem, 8 per byte, LSB first
} block_q1_sym32k;                           // 38 bytes / 256 elements = 1.1875 bpw
static_assert(sizeof(block_q1_sym32k) == sizeof(ggml_half) + QK1_SYM32K_SUPER/2 + QK1_SYM32K/8, "wrong q1_sym32k block size/padding");
#define QR1_SYM32K 8                             // elems per qs byte
#define QI1_SYM32K (QK1_SYM32K / (4 * QR1_SYM32K)) // 8 x 32-bit words of qs; one word == one sub-block

// Q2_SYM32K4: 2-bit symmetric K-scale type (type id 56). 8 sub-blocks of 32;
// the sub-scales are 4-bit UNSIGNED nibbles (even index in the low nibble,
// exactly Q1_SYM32K's convention). value = (code - 1.5) * d * sc4[s].
// qs is SEQUENTIAL LSB-first: byte b holds elems 4b..4b+3 at shifts 0/2/4/6.
// 70 bytes / 256 = 2.1875 bpw.
#define QK2_SYM32K_SUB   32
#define QK2_SYM32K_SUPER 8
#define QK2_SYM32K (QK2_SYM32K_SUB * QK2_SYM32K_SUPER) // 256 elements
#define QR2_SYM32K 4                             // elems per qs byte
#define QI2_SYM32K (QK2_SYM32K / (4 * QR2_SYM32K)) // 16 x 32-bit words of qs; two words == one sub-block
typedef struct {
    ggml_half d;                             // fp16 super-scale
    uint8_t   scales[QK2_SYM32K_SUPER / 2];  // 8 x 4-bit unsigned sub-scales
    uint8_t   qs[QK2_SYM32K / 4];            // 2 bits/elem, shifts 0/2/4/6
} block_q2_sym32k4;
static_assert(sizeof(block_q2_sym32k4) == sizeof(ggml_half) + QK2_SYM32K_SUPER/2 + QK2_SYM32K/4, "wrong q2_sym32k4 block size/padding");

// Ternary (levels -1/0/+1) with mask+sign coding -- "Qt_sym_ms_g32_sq4_sg8".
// Codes are not a fixed width: a 1-bit zero/non-zero mask per weight, plus ONE
// sign bit per non-zero. Exactly QKT_MS32K_NZTOT non-zeros per block keeps the
// sign field fixed-size, so blocks have a constant stride and need no offset
// table -- that budget is what makes the coding affordable.
// A sub-block's signs do NOT start at a fixed offset: the budget is per BLOCK,
// so sub-blocks vary in how many non-zeros they use. Decoders walk the mask in
// order advancing a running sign index (vectorised: popcount of all preceding
// mask bits in the block). Everything needed is inside the block.
// 2 + 4 + 32 + 16 = 54 bytes / 256 weights = 1.6875 bpw.
#define QKT_MS32K_SUB   32
#define QKT_MS32K_SUPER 8
#define QKT_MS32K (QKT_MS32K_SUB * QKT_MS32K_SUPER) // 256 elements
#define QKT_MS32K_NZ    16
#define QKT_MS32K_NZTOT (QKT_MS32K_NZ * QKT_MS32K_SUPER) // 128 non-zeros per block, exact
typedef struct {
    ggml_half d;                           // fp16 super-scale
    uint8_t   scales[QKT_MS32K_SUPER / 2]; // 8 x 4-bit unsigned sub-scales
    uint8_t   mask[QKT_MS32K / 8];         // 256 bits, 1 = non-zero, LSB-first
    uint8_t   signs[QKT_MS32K_NZTOT / 8];  // 128 bits, 1 = negative, LSB-first
} block_qt_ms32k4;
static_assert(sizeof(block_qt_ms32k4) == sizeof(ggml_half) + QKT_MS32K_SUPER/2 + QKT_MS32K/8 + QKT_MS32K_NZTOT/8, "wrong qt_ms32k4 block size/padding");

// Q4_SYM16K: 4-bit symmetric K-scale type. 8 sub-blocks of 16 (NOT 32);
// nibble order is PAIRWISE: byte j of a sub-block holds elements 2j (low
// nibble) and 2j+1 (high). value = (code - 7.5) * d * scales[s] (int8 sub-scales).
#define QK4_SYM16K_SUB   16
#define QK4_SYM16K_SUPER 8
#define QK4_SYM16K (QK4_SYM16K_SUB * QK4_SYM16K_SUPER) // 128 elements
typedef struct {
    ggml_half d;                             // fp16 super-scale
    int8_t    scales[QK4_SYM16K_SUPER];      // 8 x int8 sub-scales
    uint8_t   qs[QK4_SYM16K / 2];            // 4 bits/elem, pairwise, low nibble first
} block_q4_sym16k;                           // 74 bytes / 128 elements = 4.625 bpw
static_assert(sizeof(block_q4_sym16k) == sizeof(ggml_half) + QK4_SYM16K_SUPER + QK4_SYM16K/2, "wrong q4_sym16k block size/padding");
#define QR4_SYM16K 2                             // elems per qs byte
#define QI4_SYM16K (QK4_SYM16K / (4 * QR4_SYM16K)) // 16 x 32-bit words of qs; two words == one sub-block

// 4-bit symmetric, group=16, UE4M3 scale. One block covers TWO groups (32
// elements) so the block size is a multiple of QK8_1 and the type can ride
// the standard MMVQ machinery. Group g (g=0,1): scale d[g]; byte qs[8g+j]
// holds elem(16g+j) in the low nibble and elem(16g+8+j) in the high nibble.
// W = ue4m3(d[g]) * (code - 7.5).
#define QK4_SYM16 32
typedef struct {
    uint8_t d[2];               // UE4M3 scales, one per 16-element group
    uint8_t qs[QK4_SYM16 / 2];  // packed nibbles, per-group q4_0-style split
} block_q4_sym16;
static_assert(sizeof(block_q4_sym16) == 2 + QK4_SYM16 / 2, "wrong q4_sym16 block size/padding");
