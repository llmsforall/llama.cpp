...
// Millie fork: registry of canonical custom-type names for the
// `general.tensor_types` metadata convention. A GGUF may carry string
// entries "id=name" declaring what the custom numeric type ids used in its
// tensor headers mean. Ids are per-file handles; the NAME permanently
// identifies the byte layout. The loader translates declared ids to this
// build's enum by name, so files stay readable across any renumbering.
// Names are append-only and frozen once a file has shipped with them.
static enum ggml_type gguf_custom_type_from_name(const std::string & name) {
    static const std::map<std::string, enum ggml_type> registry = {
        { "Q1_sym_g32_sq4_sg8", GGML_TYPE_Q1_SYM32K  },
        { "Q2_sym_g32_sq4_sg8", GGML_TYPE_Q2_SYM32K4 },
        { "Q4_sym_g16_sq8_sg8", GGML_TYPE_Q4_SYM16K  },
        { "Qt_sym_ms_g32_sq4_sg8", GGML_TYPE_QT_MS32K4 },
    };
    const auto it = registry.find(name);
    return it == registry.end() ? GGML_TYPE_COUNT : it->second;
}
...
struct gguf_context * gguf_init_from_file_ptr(FILE * file, struct gguf_init_params params) {
    ...
    // Millie fork: parse the per-file custom tensor-type mapping
    // (`general.tensor_types`, entries "id=name") before tensor infos are
    // read, so declared ids can be translated to this build's enum prior to
    // type validation. Ids whose names this build does not know only fail if
    // a tensor actually uses them.
    std::map<int32_t, enum ggml_type> type_remap;
    std::map<int32_t, std::string>    type_unknown;
    for (const auto & kv : ctx->kv) {
        if (kv.get_key() != "general.tensor_types" || kv.get_type() != GGUF_TYPE_STRING) {
            continue;
        }
        for (const std::string & entry : kv.data_string) {
            const size_t eq = entry.find('=');
            char * end = nullptr;
            const long id = eq == std::string::npos ? 0 : strtol(entry.c_str(), &end, 10);
            if (eq == std::string::npos || eq == 0 || eq + 1 >= entry.size() || end != entry.c_str() + eq) {
                GGML_LOG_WARN("%s: ignoring malformed general.tensor_types entry '%s'\n", __func__, entry.c_str());
                continue;
            }
            const std::string name = entry.substr(eq + 1);
            const enum ggml_type t = gguf_custom_type_from_name(name);
            if (t == GGML_TYPE_COUNT) {
                type_unknown.emplace((int32_t) id, name);
            } else {
                if ((int32_t) id != (int32_t) t) {
                    GGML_LOG_INFO("%s: general.tensor_types: declared type id %ld = %s -> internal type %d\n",
                        __func__, id, name.c_str(), (int) t);
                }
                type_remap.emplace((int32_t) id, t);
            }
        }
    }
    ...
    for (int64_t i = 0; ok && i < n_tensors; ++i) {
        ...
        // tensor type
        {
            ...
            // Millie fork: translate declared custom type ids (see the
            // general.tensor_types parse above) before validation.
            if (ok) {
                const auto rit = type_remap.find((int32_t) info.t.type);
                if (rit != type_remap.end()) {
                    info.t.type = rit->second;
                } else {
                    const auto uit = type_unknown.find((int32_t) info.t.type);
                    if (uit != type_unknown.end()) {
                        GGML_LOG_ERROR("%s: tensor '%s' uses type '%s' (declared id %d), which this build does not support\n",
                            __func__, info.t.name, uit->second.c_str(), (int) info.t.type);
                        ok = false;
                        break;
                    }
                }
