...
//========================- 1.5-bit affine (de)-quantization

void quantize_row_q1_affine_ref(const float * GGML_RESTRICT x, block_q1_affine * GGML_RESTRICT y, int64_t k) {
    assert(k % QK1_AFFINE == 0);
    const int nb = k / QK1_AFFINE;

    uint8_t L[QK1_AFFINE];
    uint8_t Laux[QK1_AFFINE];
    float weights[QK1_AFFINE];
    for (int j = 0; j < QK1_AFFINE; ++j) weights[j] = 1.0f;

    for (int i = 0; i < nb; i++) {
        const float * xb = x + i * QK1_AFFINE;
        float the_min = 0.0f;
        // nmax=1: literal 1-bit codes. rmin/rdelta/nstep match make_qkx2_quants's
        // own K-quant callers (a modest local search around the initial estimate).
        float scale = make_qkx2_quants(QK1_AFFINE, 1, xb, weights, L, &the_min, Laux,
                                        -1.0f, 0.1f, 20, false);
        // make_qkx2_quants returns *the_min = -min (GGUF storage convention: value
        // to subtract at dequant time), and min <= 0 always, so the_min >= 0.
        const uint8_t d_code = ggml_fp32_to_ue4m3(scale > 0.0f ? scale * 2.0f : 0.0f);
        const uint8_t m_code = ggml_fp32_to_ue4m3(the_min > 0.0f ? the_min * 2.0f : 0.0f);

        y[i].d = d_code;
        y[i].m = m_code;
        memset(y[i].qs, 0, sizeof(y[i].qs));
        for (int j = 0; j < QK1_AFFINE; ++j) {
            if (L[j]) {
                y[i].qs[j / 8] |= (1 << (j % 8));
            }
        }
...
void dequantize_row_q1_affine(const block_q1_affine * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK1_AFFINE == 0);
    const int nb = k / QK1_AFFINE;

    for (int i = 0; i < nb; i++) {
        // NOTE: ggml_ue4m3_to_fp32 already applies its own *0.5f internally
        // (to match the kvalues_mxfp4 convention); do not halve again here.
        const float scale = ggml_ue4m3_to_fp32(x[i].d);
        const float the_min = ggml_ue4m3_to_fp32(x[i].m); // >= 0, subtract at dequant
        for (int j = 0; j < QK1_AFFINE; ++j) {
            const int code = (x[i].qs[j / 8] >> (j % 8)) & 1;
            y[i * QK1_AFFINE + j] = scale * (float) code - the_min;
        }
    }
}

size_t quantize_q1_affine(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    GGML_UNUSED(quant_weights); // no imatrix support for this prototype type
    const size_t row_size = ggml_row_size(GGML_TYPE_Q1_AFFINE, n_per_row);
    char * qrow = (char *) dst;
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_q1_affine_ref(src, (block_q1_affine *) qrow, n_per_row);
        src += n_per_row;
        qrow += row_size;
    }
...
// Symmetric-only (no min/affine), fp8 (UE4M3) scale = mean(|x|) over the group
// This is the closed-form least-squares-optimal scale given sign-fixed codes:
// code = sign(x), scale = mean(|x|).
#define Q1_SYM_IMPL(GROUP, BLOCK_T) \
void quantize_row_##BLOCK_T##_ref(const float * GGML_RESTRICT x, BLOCK_T * GGML_RESTRICT y, int64_t k) { \
    assert(k % (GROUP) == 0); \
    const int nb = k / (GROUP); \
    for (int i = 0; i < nb; i++) { \
        const float * xb = x + i * (GROUP); \
        float sum_abs = 0.0f; \
        for (int j = 0; j < (GROUP); ++j) sum_abs += fabsf(xb[j]); \
        const float scale = sum_abs / (GROUP); \
        y[i].d = ggml_fp32_to_ue4m3(scale > 0.0f ? scale * 2.0f : 0.0f); \
        memset(y[i].qs, 0, sizeof(y[i].qs)); \
        for (int j = 0; j < (GROUP); ++j) { \
            if (xb[j] >= 0.0f) y[i].qs[j / 8] |= (1 << (j % 8)); \
        } \
    } \
} \
void dequantize_row_##BLOCK_T(const BLOCK_T * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) { \
    assert(k % (GROUP) == 0); \
    const int nb = k / (GROUP); \
    for (int i = 0; i < nb; i++) { \
        const float scale = ggml_ue4m3_to_fp32(x[i].d); \
        for (int j = 0; j < (GROUP); ++j) { \
            const int code = (x[i].qs[j / 8] >> (j % 8)) & 1; \
            y[i * (GROUP) + j] = code ? scale : -scale; \
        } \
    } \
} \
size_t quantize_##BLOCK_T(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) { \
    GGML_UNUSED(quant_weights); \
    const size_t row_size = n_per_row / (GROUP) * sizeof(BLOCK_T); \
    char * qrow = (char *) dst; \
    for (int64_t row = 0; row < nrow; ++row) { \
        quantize_row_##BLOCK_T##_ref(src, (BLOCK_T *) qrow, n_per_row); \
        src += n_per_row; \
        qrow += row_size; \
    } \
    return nrow * row_size; \
...
Q1_SYM_IMPL(QK1_SYM16, block_q1_sym16)
Q1_SYM_IMPL(QK1_SYM64, block_q1_sym64)

// Symmetric-only (no min), 4-level {-1.5,-0.5,+0.5,+1.5}*scale, group=16.
// Iterative: sweep candidate scales, round to nearest of the 4 fixed levels,
// then re-solve the closed-form least-squares-optimal (pure multiplicative,
// no offset) scale for those codes -- same shape as make_qkx2_quants's inner
// loop but with no min term at all (not just a min forced to 0).
static const float Q2_SYM_LEVELS[4] = { -1.5f, -0.5f, 0.5f, 1.5f };

void quantize_row_block_q2_sym16_ref(const float * GGML_RESTRICT x, block_q2_sym16 * GGML_RESTRICT y, int64_t k) {
    assert(k % QK2_SYM16 == 0);
    const int nb = k / QK2_SYM16;
    uint8_t L[QK2_SYM16];
    uint8_t Laux[QK2_SYM16];

    for (int i = 0; i < nb; i++) {
        const float * xb = x + i * QK2_SYM16;

        float amax = 0.0f;
        for (int j = 0; j < QK2_SYM16; ++j) amax = MAX(amax, fabsf(xb[j]));
        if (amax == 0.0f) {
            y[i].d = 0;
            memset(y[i].qs, 0, sizeof(y[i].qs));
            continue;
        ...
        float best_scale = amax / 1.5f;
        float best_error = FLT_MAX;
        for (int j = 0; j < QK2_SYM16; ++j) {
            int l = nearest_int(xb[j] / best_scale + 1.5f);
            L[j] = MAX(0, MIN(3, l));
        }

        for (int is = -8; is <= 8; ++is) {
            float cand_scale = best_scale * (1.0f + 0.05f * is);
            if (cand_scale <= 0.0f) continue;
            float sum_lx = 0.0f, sum_l2 = 0.0f;
            for (int j = 0; j < QK2_SYM16; ++j) {
                int l = nearest_int(xb[j] / cand_scale + 1.5f);
                l = MAX(0, MIN(3, l));
                Laux[j] = l;
                const float lvl = Q2_SYM_LEVELS[l];
                sum_lx += lvl * xb[j];
                sum_l2 += lvl * lvl;
            }
            if (sum_l2 <= 0.0f) continue;
            const float this_scale = sum_lx / sum_l2;
            float err = 0.0f;
            for (int j = 0; j < QK2_SYM16; ++j) {
                const float diff = this_scale * Q2_SYM_LEVELS[Laux[j]] - xb[j];
                err += diff * diff;
            }
            if (err < best_error) {
                best_error = err;
                for (int j = 0; j < QK2_SYM16; ++j) L[j] = Laux[j];
            }
        }

        // final closed-form refit against the winning code assignment
        float sum_lx = 0.0f, sum_l2 = 0.0f;
        for (int j = 0; j < QK2_SYM16; ++j) {
            const float lvl = Q2_SYM_LEVELS[L[j]];
            sum_lx += lvl * xb[j];
            sum_l2 += lvl * lvl;
        }
        const float final_scale = sum_l2 > 0.0f ? sum_lx / sum_l2 : 0.0f;

        y[i].d = ggml_fp32_to_ue4m3(final_scale > 0.0f ? final_scale * 2.0f : 0.0f);
        memset(y[i].qs, 0, sizeof(y[i].qs));
        for (int j = 0; j < QK2_SYM16; ++j) {
            y[i].qs[j / 4] |= (uint8_t)(L[j] << ((j % 4) * 2));
        }
    }
}

void dequantize_row_block_q2_sym16(const block_q2_sym16 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK2_SYM16 == 0);
    const int nb = k / QK2_SYM16;
    for (int i = 0; i < nb; i++) {
        const float scale = ggml_table_ue4m3_f32[x[i].d]; // table: see dequantize_row_q4_sym16
        for (int j = 0; j < QK2_SYM16; ++j) {
            const int code = (x[i].qs[j / 4] >> ((j % 4) * 2)) & 3;
            y[i * QK2_SYM16 + j] = scale * Q2_SYM_LEVELS[code];
        }
    }
}

void quantize_row_q4_sym16_ref(const float * GGML_RESTRICT x, block_q4_sym16 * GGML_RESTRICT y, int64_t k) {
    assert(k % QK4_SYM16 == 0);
    const int nb = k / QK4_SYM16;
    for (int i = 0; i < nb; i++) {
        for (int g = 0; g < 2; ++g) {
            const float * xg = x + i*QK4_SYM16 + g*16;
            float amax = 0.0f;
            for (int j = 0; j < 16; ++j) amax = MAX(amax, fabsf(xg[j]));
            uint8_t codes[16];
            if (amax == 0.0f) {
                y[i].d[g] = 0;
                memset(codes, 0, sizeof(codes));
            } else {
                // Round-to-nearest reference quantizer for this block layout.
                y[i].d[g] = ggml_fp32_to_ue4m3(2.0f * amax / 7.5f);
                const float dq = ggml_ue4m3_to_fp32(y[i].d[g]);
                for (int j = 0; j < 16; ++j) {
                    int l = dq > 0.0f ? nearest_int(xg[j] / dq + 7.5f) : 0;
                    codes[j] = (uint8_t) MAX(0, MIN(15, l));
                }
            }
            for (int j = 0; j < 8; ++j) {
                y[i].qs[8*g + j] = codes[j] | (codes[j + 8] << 4);
            }
        }
    }
}

void dequantize_row_q4_sym16(const block_q4_sym16 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK4_SYM16 == 0);
    const int nb = k / QK4_SYM16;
    for (int i = 0; i < nb; i++) {
        for (int g = 0; g < 2; ++g) {
            const float d = ggml_table_ue4m3_f32[x[i].d[g]]; // table: ldexpf in ggml_ue4m3_to_fp32 is too slow for the BLAS dequant path
            for (int j = 0; j < 8; ++j) {
                const uint8_t b = x[i].qs[8*g + j];
                y[i*QK4_SYM16 + 16*g + j]     = d * ((b & 0x0F) - 7.5f);
                y[i*QK4_SYM16 + 16*g + j + 8] = d * ((b >>   4) - 7.5f);
            }
        }
    }
}

// ---- K-scale SYM types (Q1_SYM32K, Q4_SYM16K) ----

void dequantize_row_q1_sym32k(const block_q1_sym32k * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK1_SYM32K == 0);
    const int nb = k / QK1_SYM32K;
    for (int i = 0; i < nb; i++) {
        const float d = GGML_FP16_TO_FP32(x[i].d);
        for (int s = 0; s < QK1_SYM32K_SUPER; ++s) {
            const uint8_t sc = (s & 1) ? (x[i].scales[s >> 1] >> 4)
                                       : (x[i].scales[s >> 1] & 0x0F);
            const float h = 0.5f * d * (float) sc;
            const uint8_t * qs = x[i].qs + s*(QK1_SYM32K_SUB/8);
            float * yb = y + i*QK1_SYM32K + s*QK1_SYM32K_SUB;
            for (int j = 0; j < QK1_SYM32K_SUB; ++j) {
                yb[j] = ((qs[j/8] >> (j % 8)) & 1) ? h : -h;
            }
        }
    }
}

void quantize_row_q1_sym32k_ref(const float * GGML_RESTRICT x, block_q1_sym32k * GGML_RESTRICT y, int64_t k) {
    assert(k % QK1_SYM32K == 0);
    const int nb = k / QK1_SYM32K;
    for (int i = 0; i < nb; i++) {
        float sub[QK1_SYM32K_SUPER];
        float amax = 0.0f;
        for (int s = 0; s < QK1_SYM32K_SUPER; ++s) {
            float m = 0.0f;
            for (int j = 0; j < QK1_SYM32K_SUB; ++j) {
                const float v = fabsf(x[i*QK1_SYM32K + s*QK1_SYM32K_SUB + j]);
                if (v > m) m = v;
            }
            sub[s] = 2.0f * m; // levels are +-0.5*scale
            if (sub[s] > amax) amax = sub[s];
        }
        const float d = amax / 15.0f;
        y[i].d = GGML_FP32_TO_FP16(d);
        memset(y[i].scales, 0, sizeof(y[i].scales));
        memset(y[i].qs, 0, sizeof(y[i].qs));
        for (int s = 0; s < QK1_SYM32K_SUPER; ++s) {
            int q = d > 0.0f ? (int) roundf(sub[s] / d) : 0;
            if (q < 0)  q = 0;
            if (q > 15) q = 15;
            if (s & 1) y[i].scales[s >> 1] |= (uint8_t)(q << 4);
            else       y[i].scales[s >> 1] |= (uint8_t) q;
            uint8_t * qs = y[i].qs + s*(QK1_SYM32K_SUB/8);
            for (int j = 0; j < QK1_SYM32K_SUB; ++j) {
                if (x[i*QK1_SYM32K + s*QK1_SYM32K_SUB + j] >= 0.0f) qs[j/8] |= (uint8_t)(1u << (j % 8));
            }
        }
    }
}

size_t quantize_q1_sym32k(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    GGML_UNUSED(quant_weights);
    const size_t row_size = n_per_row / QK1_SYM32K * sizeof(block_q1_sym32k);
    char * qrow = (char *) dst;
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_q1_sym32k_ref(src, (block_q1_sym32k *) qrow, n_per_row);
        src += n_per_row;
        qrow += row_size;
    }
    return nrow * row_size;
...
void dequantize_row_q2_sym32k4(const block_q2_sym32k4 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK2_SYM32K == 0);
    const int nb = k / QK2_SYM32K;
    for (int i = 0; i < nb; i++) {
        const float d = GGML_FP16_TO_FP32(x[i].d);
        for (int s = 0; s < QK2_SYM32K_SUPER; ++s) {
            const uint8_t sc = (s & 1) ? (x[i].scales[s >> 1] >> 4) : (x[i].scales[s >> 1] & 0x0F);
            const float ds = d * (float) sc;
            float * yb = y + i*QK2_SYM32K + s*QK2_SYM32K_SUB;
            for (int j = 0; j < QK2_SYM32K_SUB/4; ++j) {
                const uint8_t b = x[i].qs[s*(QK2_SYM32K_SUB/4) + j];
                for (int q = 0; q < 4; ++q) yb[4*j + q] = ((float)((b >> (2*q)) & 3) - 1.5f) * ds;
            }
        }
    }
}

void dequantize_row_qt_ms32k4(const block_qt_ms32k4 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QKT_MS32K == 0);
    const int nb = k / QKT_MS32K;
    for (int i = 0; i < nb; i++) {
        const float d = GGML_FP16_TO_FP32(x[i].d);
        // Sign index runs across the WHOLE block: the non-zero budget is per
        // block, so a sub-block's signs do not begin at a fixed offset.
        int idx = 0;
        for (int s = 0; s < QKT_MS32K_SUPER; ++s) {
            const uint8_t sc = (s & 1) ? (x[i].scales[s >> 1] >> 4) : (x[i].scales[s >> 1] & 0x0F);
            const float ds = d * (float) sc;
            float * yb = y + i*QKT_MS32K + s*QKT_MS32K_SUB;
            for (int j = 0; j < QKT_MS32K_SUB; ++j) {
                const int bit = s*QKT_MS32K_SUB + j;
                if ((x[i].mask[bit >> 3] >> (bit & 7)) & 1) {
                    yb[j] = ((x[i].signs[idx >> 3] >> (idx & 7)) & 1) ? -ds : ds;
                    idx++;
                } else {
                    yb[j] = 0.0f;
                }
...
void quantize_row_qt_ms32k4_ref(const float * GGML_RESTRICT x, block_qt_ms32k4 * GGML_RESTRICT y, int64_t k) {
    assert(k % QKT_MS32K == 0);
    const int nb = k / QKT_MS32K;
    for (int i = 0; i < nb; i++) {
        const float * xb = x + i*QKT_MS32K;
        memset(&y[i], 0, sizeof(block_qt_ms32k4));
        // Reference allocator: keep the QKT_MS32K_NZTOT largest |w| in the block.
        float tmp[QKT_MS32K];
        for (int j = 0; j < QKT_MS32K; ++j) tmp[j] = fabsf(xb[j]);
        for (int a = 0; a < QKT_MS32K_NZTOT; ++a) {
            int best = a;
            for (int b = a+1; b < QKT_MS32K; ++b) if (tmp[b] > tmp[best]) best = b;
            const float t = tmp[a]; tmp[a] = tmp[best]; tmp[best] = t;
        }
        const float thr = tmp[QKT_MS32K_NZTOT-1];
        int kept = 0;
        for (int j = 0; j < QKT_MS32K && kept < QKT_MS32K_NZTOT; ++j) {
            if (fabsf(xb[j]) >= thr) { y[i].mask[j >> 3] |= (uint8_t)(1u << (j & 7)); kept++; }
        }
        float sub_d[QKT_MS32K_SUPER];
        float amax_of_d = 0.0f;
        for (int s = 0; s < QKT_MS32K_SUPER; ++s) {
            float sum = 0.0f; int n = 0;
            for (int j = 0; j < QKT_MS32K_SUB; ++j) {
                const int bit = s*QKT_MS32K_SUB + j;
                if ((y[i].mask[bit >> 3] >> (bit & 7)) & 1) { sum += fabsf(xb[bit]); n++; }
            }
            sub_d[s] = n ? sum / (float) n : 0.0f;
            if (sub_d[s] > amax_of_d) amax_of_d = sub_d[s];
        }
        const float d = amax_of_d / 15.0f;
        y[i].d = GGML_FP32_TO_FP16(d);
        const float id = d ? 1.0f/d : 0.0f;
        for (int s = 0; s < QKT_MS32K_SUPER; ++s) {
            int q = (int) roundf(sub_d[s] * id);
            if (q < 0) q = 0;
            if (q > 15) q = 15;
            y[i].scales[s >> 1] |= (uint8_t)(q << ((s & 1) ? 4 : 0));
        }
        int idx = 0;
        for (int j = 0; j < QKT_MS32K; ++j) {
            if ((y[i].mask[j >> 3] >> (j & 7)) & 1) {
                if (xb[j] < 0.0f) y[i].signs[idx >> 3] |= (uint8_t)(1u << (idx & 7));
                idx++;
            }
        }
    }
...
size_t quantize_qt_ms32k4(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrows, int64_t n_per_row, const float * imatrix) {
    GGML_UNUSED(imatrix);
    assert(n_per_row % QKT_MS32K == 0);
    const size_t row_size = ggml_row_size(GGML_TYPE_QT_MS32K4, n_per_row);
    char * qrow = (char *) dst;
    for (int64_t row = 0; row < nrows; ++row) {
        quantize_row_qt_ms32k4_ref(src, (block_qt_ms32k4 *) qrow, n_per_row);
        src += n_per_row;
        qrow += row_size;
    }
...
void quantize_row_q2_sym32k4_ref(const float * GGML_RESTRICT x, block_q2_sym32k4 * GGML_RESTRICT y, int64_t k) {
    assert(k % QK2_SYM32K == 0);
    const int nb = k / QK2_SYM32K;
    float sub_d[QK2_SYM32K_SUPER];
    for (int i = 0; i < nb; i++) {
        const float * xb = x + i*QK2_SYM32K;
        float amax_of_d = 0.0f;
        for (int s = 0; s < QK2_SYM32K_SUPER; ++s) {
            float amax = 0.0f;
            for (int j = 0; j < QK2_SYM32K_SUB; ++j) {
                const float a = fabsf(xb[s*QK2_SYM32K_SUB + j]);
                if (a > amax) amax = a;
            }
            sub_d[s] = amax / 1.5f;
            if (sub_d[s] > amax_of_d) amax_of_d = sub_d[s];
        }
        const float sd  = amax_of_d / 15.0f;
        const float isd = sd > 0.0f ? 1.0f/sd : 0.0f;
        y[i].d = GGML_FP32_TO_FP16(sd);
        const float sd_r = GGML_FP16_TO_FP32(y[i].d);
        memset(y[i].scales, 0, sizeof(y[i].scales));
        for (int s = 0; s < QK2_SYM32K_SUPER; ++s) {
            int q = (int)roundf(sub_d[s]*isd);
            if (q < 0)  q = 0;
            if (q > 15) q = 15;
            if (s & 1) y[i].scales[s >> 1] |= (uint8_t)(q << 4);
            else       y[i].scales[s >> 1] |= (uint8_t) q;
            sub_d[s]  = sd_r * (float) q;
        }
        for (int s = 0; s < QK2_SYM32K_SUPER; ++s) {
            const float id = sub_d[s] > 0.0f ? 1.0f/sub_d[s] : 0.0f;
            for (int j = 0; j < QK2_SYM32K_SUB/4; ++j) {
                uint8_t byte = 0;
                for (int q = 0; q < 4; ++q) {
                    int v = (int)roundf(xb[s*QK2_SYM32K_SUB + 4*j + q]*id + 1.5f);
                    if (v < 0) v = 0;
                    if (v > 3) v = 3;
                    byte |= (uint8_t)(v << (2*q));
                }
                y[i].qs[s*(QK2_SYM32K_SUB/4) + j] = byte;
            }
        }
    }
}

size_t quantize_q2_sym32k4(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    GGML_UNUSED(quant_weights);
    const size_t row_size = n_per_row / QK2_SYM32K * sizeof(block_q2_sym32k4);
    char * qrow = (char *) dst;
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_q2_sym32k4_ref(src, (block_q2_sym32k4 *) qrow, n_per_row);
        src += n_per_row;
        qrow += row_size;
    }
    return nrow * row_size;
...
void dequantize_row_q4_sym16k(const block_q4_sym16k * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK4_SYM16K == 0);
    const int nb = k / QK4_SYM16K;
    for (int i = 0; i < nb; i++) {
        const float d = GGML_FP16_TO_FP32(x[i].d);
        for (int s = 0; s < QK4_SYM16K_SUPER; ++s) {
            const float ds = d * (float) x[i].scales[s];
            const uint8_t * qs = x[i].qs + s*(QK4_SYM16K_SUB/2);
            float * yb = y + i*QK4_SYM16K + s*QK4_SYM16K_SUB;
            // PAIRWISE: byte j holds elements 2j and 2j+1 (NOT the half-split order)
            for (int j = 0; j < QK4_SYM16K_SUB/2; ++j) {
                const uint8_t b = qs[j];
                yb[2*j    ] = ((float)(b & 0x0F) - 7.5f) * ds;
                yb[2*j + 1] = ((float)(b >>   4) - 7.5f) * ds;
            }
        }
    }
}

void quantize_row_q4_sym16k_ref(const float * GGML_RESTRICT x, block_q4_sym16k * GGML_RESTRICT y, int64_t k) {
    assert(k % QK4_SYM16K == 0);
    const int nb = k / QK4_SYM16K;
    for (int i = 0; i < nb; i++) {
        float sub[QK4_SYM16K_SUPER];
        float amax = 0.0f;
        for (int s = 0; s < QK4_SYM16K_SUPER; ++s) {
            float m = 0.0f;
            for (int j = 0; j < QK4_SYM16K_SUB; ++j) {
                const float v = fabsf(x[i*QK4_SYM16K + s*QK4_SYM16K_SUB + j]);
                if (v > m) m = v;
            }
            sub[s] = m / 7.5f;
            if (sub[s] > amax) amax = sub[s];
        ...
        for (int s = 0; s < QK4_SYM16K_SUPER; ++s) {
            ...
            if (q < 0)   q = 0;
            if (q > 127) q = 127;
            y[i].scales[s] = (int8_t) q;
            const float ds = d * (float) q;
            uint8_t * qs = y[i].qs + s*(QK4_SYM16K_SUB/2);
            for (int j = 0; j < QK4_SYM16K_SUB/2; ++j) {
                int lo = 0, hi = 0;
                if (ds > 0.0f) {
                    lo = (int) roundf(x[i*QK4_SYM16K + s*QK4_SYM16K_SUB + 2*j    ] / ds + 7.5f);
                    hi = (int) roundf(x[i*QK4_SYM16K + s*QK4_SYM16K_SUB + 2*j + 1] / ds + 7.5f);
                }
...
size_t quantize_q4_sym16k(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    GGML_UNUSED(quant_weights);
    const size_t row_size = n_per_row / QK4_SYM16K * sizeof(block_q4_sym16k);
    char * qrow = (char *) dst;
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_q4_sym16k_ref(src, (block_q4_sym16k *) qrow, n_per_row);
        src += n_per_row;
        qrow += row_size;
    }
    return nrow * row_size;
...
size_t quantize_block_q2_sym16(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    GGML_UNUSED(quant_weights);
    const size_t row_size = n_per_row / QK2_SYM16 * sizeof(block_q2_sym16);
    char * qrow = (char *) dst;
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_block_q2_sym16_ref(src, (block_q2_sym16 *) qrow, n_per_row);
        src += n_per_row;
        qrow += row_size;
    }
    return nrow * row_size;
...
size_t quantize_q4_sym16(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    GGML_UNUSED(quant_weights);
    const size_t row_size = ggml_row_size(GGML_TYPE_Q4_SYM16, n_per_row);
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_q4_sym16_ref(src + row*n_per_row, (block_q4_sym16 *)((char *) dst + row*row_size), n_per_row);
    }
    return nrow * row_size;
}
...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_I32:
        case GGML_TYPE_I64:
        case GGML_TYPE_Q1_AFFINE:
        case GGML_TYPE_Q1_SYM16:
        case GGML_TYPE_Q1_SYM64:
        case GGML_TYPE_Q2_SYM16:
        case GGML_TYPE_Q4_SYM16:
            // scale/min are raw uint8 fp8 (UE4M3) codes, codes are packed bits --
            // no field can represent NaN/Inf, nothing to validate.
            break;
        case GGML_TYPE_Q1_SYM32K:
            {
                VALIDATE_ROW_DATA_D_F16_IMPL(block_q1_sym32k, data, nb);
            } break;
        case GGML_TYPE_Q4_SYM16K:
            {
                VALIDATE_ROW_DATA_D_F16_IMPL(block_q4_sym16k, data, nb);
            } break;
        default:
            {
                fprintf(stderr, "%s: invalid type %d\n", __func__, type);
