...
extern "C" {
    ...
    enum ggml_type {
        ...
        GGML_TYPE_Q1_0    = 41,
        // 42-46 are reserved. Files already exist carrying these numeric
        // type IDs; do NOT assign new types to them.
        GGML_TYPE_Q1_AFFINE = 47, // 1.5 bpw: 1-bit affine (scale+min), group=32, UE4M3 scale/min
        GGML_TYPE_Q1_SYM16 = 48,  // 1.5 bpw: 1-bit symmetric, group=16, UE4M3 scale
        GGML_TYPE_Q1_SYM64 = 49,  // 1.125 bpw: 1-bit symmetric, group=64, UE4M3 scale
        GGML_TYPE_Q2_SYM16 = 50,  // 2.5 bpw: 2-bit (4-level) symmetric, group=16, UE4M3 scale
        GGML_TYPE_Q4_SYM16 = 51,  // 4.5 bpw: 4-bit (16-level) symmetric, group=16, UE4M3 scale; 32-elem blocks pair two groups
        // 52 is reserved (removed type); do not reuse.
        // 53 is reserved; do not reuse.
        GGML_TYPE_Q1_SYM32K = 54, // 1.1875 bpw: 1-bit symmetric, sub-block=32, 4-bit unsigned sub-scales + fp16 super-scale per 8 subs
        GGML_TYPE_Q4_SYM16K = 55, // 4.625 bpw: 4-bit symmetric, sub-block=16 PAIRWISE nibbles, int8 sub-scales + fp16 super-scale per 8 subs
        GGML_TYPE_Q2_SYM32K4 = 56, // 2.1875 bpw: 2-bit symmetric, sub-block=32, 4-bit UNSIGNED sub-scales + fp16 super-scale per 8 subs
        GGML_TYPE_QT_MS32K4 = 57, // 1.6875 bpw: TERNARY (-1/0/+1), mask+sign coding, sub-block=32, 128 non-zeros per 256-weight block, 4-bit UNSIGNED sub-scales + fp16 super-scale per 8 subs
        GGML_TYPE_COUNT   = 58,
    };
