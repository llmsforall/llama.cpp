...
class GGMLQuantizationType(IntEnum):
    ...
    NVFP4   = 40
    Q1_0    = 41
    # 42-46 are reserved and must not be reused (matches ggml/include/ggml.h).
    Q1_AFFINE = 47  # 1.5 bpw: 1-bit affine (scale+min), group=32, UE4M3 scale/min
    Q1_SYM16  = 48  # 1.5 bpw: 1-bit symmetric, group=16, UE4M3 scale
    Q1_SYM64  = 49  # 1.125 bpw: 1-bit symmetric, group=64, UE4M3 scale
    Q2_SYM16  = 50  # 2.5 bpw: 2-bit symmetric, group=16, UE4M3 scale
    Q4_SYM16  = 51  # 4.5 bpw: 4-bit symmetric, group=16, UE4M3 scale; 32-elem block pairs two groups
    # 52 is reserved (removed type); do not reuse.
    Q1_SYM32K = 54  # 1.1875 bpw: 1-bit symmetric, sub=32, int4 sub-scales + fp16 super per 8
    Q4_SYM16K = 55  # 4.625 bpw: 4-bit symmetric, sub=16 pairwise, int8 sub-scales + fp16 super per 8
    Q2_SYM32K4 = 56  # 2.1875 bpw: 2-bit symmetric, sub=32, 4-bit unsigned sub-scales + fp16 super per 8
    QT_MS32K4  = 57  # 1.6875 bpw: TERNARY (-1/0/+1) mask+sign, sub=32, 128 nonzeros / 256-weight block, 4-bit unsigned sub-scales + fp16 super per 8
...
class LlamaFileType(IntEnum):
    ...
    # 52 is reserved (removed type); do not reuse.
    MOSTLY_Q1_SYM32K     = 54  # except 1d tensors
    MOSTLY_Q4_SYM16K     = 55  # except 1d tensors
    MOSTLY_Q2_SYM32K4    = 56  # except 1d tensors
...
GGML_QUANT_SIZES: dict[GGMLQuantizationType, tuple[int, int]] = {
    ...
    GGMLQuantizationType.Q1_AFFINE: (32, 2 + 4),
    GGMLQuantizationType.Q1_SYM16:  (16, 1 + 2),
    GGMLQuantizationType.Q1_SYM64:  (64, 1 + 8),
    GGMLQuantizationType.Q2_SYM16:  (16, 1 + 4),
    GGMLQuantizationType.Q4_SYM16:  (32, 2 + 16),
    GGMLQuantizationType.Q1_SYM32K: (256, 2 + 4 + 32),
    GGMLQuantizationType.QT_MS32K4: (256, 2 + 4 + 32 + 16),   # 54 B: super-scale, 8x int4 sub-scales, 256-bit mask, 128-bit signs
    GGMLQuantizationType.Q4_SYM16K: (128, 2 + 8 + 64),
    GGMLQuantizationType.Q2_SYM32K4: (256, 2 + 4 + 64),
}
